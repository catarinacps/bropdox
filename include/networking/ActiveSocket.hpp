...
public:
    /**
     * Tries to read the socket for a sizeof(T) bytes datagram. This method is a
     * blocking call if there's no data in the socket. If theres more bytes in
     ...
     * As a warning, there's maybe a chance that the whole package didn't got
