...
bool Client::parse_input(std::vector<std::string> tokens)
{
    ...
    if (command == "connect") {
        ...
        return this->send_handshake(req::login);
    } else {
        std::cout << "usage:\n"
                  << "login <hostname> <port>\n"
                  << "upload <file path>\n"
...
bool Client::send_file(char const* file)
{
    ...
    auto packets = this->file_handler.get_file(file, file_size_in_packets);
    ...
    auto returned_ack = this->sock_handler_req.wait_packet(sizeof(ack_t));
    ...
    for (auto const& packet : packets) {
        this->sock_handler_req.send_packet(packet.get(), sizeof(packet_t));
        usleep(15);
    }
    ...
    ack = convert_to_ack(returned_ack.get());
...
bool Client::get_file(char const* file)
{
    ...
    auto received_packet = this->sock_handler_req.wait_packet(sizeof(file_data_t));
    auto received_finfo = convert_to_file_data(received_packet.get());
    ...
    for (auto& data : recv_file) {
        auto received_packet = this->sock_handler_req.wait_packet(sizeof(packet_t));
...
bool Client::send_handshake(req request)
{
    ...
    handshake_t hand(request, this->userid.c_str(), this->device);
    ...
    auto syn_data = this->sock_handler_server.wait_packet(sizeof(syn_t));
    auto syn = convert_to_syn(syn_data.get());
